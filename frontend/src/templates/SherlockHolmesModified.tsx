...
const SherlockHolmesModified: React.FC<TemplateProps> = ({ data, sectionOrder, allSections }) => {
    ...
    const MainHeading: React.FC<{ children: React.ReactNode }> = ({ children }) => (
        <h2 className="text-sm font-bold uppercase border-b-2 border-black pb-1 mb-3 mt-4 text-gray-900">{children}</h2>
    );
    ...
    const sectionMap: { [key: string]: React.ReactNode | null } = {
        ...
        summary: summary ? (
            <div className='p-4'>
                ...
                <p className="text-justify text-xs leading-snug text-gray-200">{summary}</p>
            </div>
        ) : null,
                ...
                {experience.map(item => (
                    <div key={item.id}>
                        <div className="flex justify-between items-center text-[10px] text-gray-800">
                            <h3 className="font-bold uppercase tracking-wider text-gray-900">{item.company}</h3>
                            <p className="font-medium whitespace-nowrap">{formatDate(item.startDate)} - {formatDate(item.endDate)}</p>
                        </div>
                        <p className='text-xs font-semibold italic mb-1 text-gray-700'>{item.title}</p>
        ...
        skills: skills.length > 0 ? (
            <div className="grid grid-cols-2 gap-y-3 gap-x-6">
                <div className="col-span-2"><MainHeading>Skills</MainHeading></div>
        ...
        references: references.trim() ? (
            <div className='p-4 pt-0 text-white'>
                ...
                <p className="text-xs text-gray-200">{references}</p>
            </div>
        ) : null,
        ...Object.keys(custom || {}).reduce((acc, id) => {
            ...
            if (config) {
                acc[id] = (
                    <div className='p-4 pt-0 text-white'>
                        <SidebarHeading>{config.name}</SidebarHeading>
                        <p className="text-xs text-gray-200">{custom[id]}</p>
                    </div>
                );
            }
            return acc;
    ...
    const sidebarSections = ['summary', 'achievements', 'interests', 'languages', 'references'];
    const mainSections = sectionOrder.filter(id => id !== 'personal' && !sidebarSections.includes(id));


    return (
        <div className="max-w-4xl mx-auto shadow-lg flex text-gray-800 font-sans leading-tight bg-white min-h-[1100px]">
